...
def bootstrap_estimation(y,  estimator='GMM', opt_bootstrap=dict(), opt_redform=dict(),
    ...
    e = out_svar['e']
    num_cols = e.shape[1]  # Get the number of columns in 'e'
    DepPoint = []
    # Iterate through all pairs of columns in 'e'
    for i in range(num_cols):
        for j in range(i + 1, num_cols):
            # Calculate the mean squared product for the current pair of columns
            dep_ij = np.mean(e[:, i] ** 2 * e[:, j] ** 2)

            # Append the result to the DepPoint list
            DepPoint.append(dep_ij)

    # Convert the list to a NumPy array for further analysis
    DepPoint = np.array(DepPoint)


    skew = scipy.stats.skew(e, axis=0)
    kurt = scipy.stats.kurtosis(e, axis=0,fisher=False)
    JBTest = np.zeros(num_cols)
    for i in range(num_cols):
        JBTest[i] = np.round(scipy.stats.jarque_bera(e[:, i])[1], 3)
    nG = np.concatenate([skew, kurt, JBTest])

    # get IRF
    out_irf = SVAR.SVARbasics.get_IRF(out_svar['B_est'], out_redform['AR'],   **opt_irf)

    return out_redform, out_svar, out_irf, out_svar['options'], DepPoint, nG
...
def bootstrap_SVAR(y, estimator='GMM',
    ...
    out_redform, out_svar, out_irf, opt_SVAR, DepPoint, nG = bootstrap_estimation(y,  estimator,
    ...
    def bootstrap_iter(y, estimator, options_bootstrap, options_redform, prepOptions, opt_SVAR,
        ...
        out_redform_iter, out_svar_iter, out_irf_iter, _, DepPoint_iter, nG_iter = bootstrap_estimation(y_sim,  estimator,
                                                                                options_bootstrap,
        ...
        return out_redform_iter['AR'], out_svar_iter['B_est'], out_irf_iter, DepPoint_iter, nG_iter

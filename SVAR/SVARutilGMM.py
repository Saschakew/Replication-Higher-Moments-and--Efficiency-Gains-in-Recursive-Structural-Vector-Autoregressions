...
# def get_f(y, b, restrictions, moments, moments_powerindex, AR=[],     whiten=False, blocks=False  ):
#     lags = np.shape(AR)[2]
#
#     def get_u(y, AR):
#         # if not AR:
#         #     return y  # If AR is empty, return y as-is
#
#         T, n = np.shape(y)
#         # lags = np.shape(AR)[2]
#
...
#             # tmpsum = np.sum(np.matmul(AR[:, :, :], y[t - lags:t, :]), axis=0)
#             tmpsum = 0
#             for j in range(lags):
#                 # if t - j > 0:
#                 tmpsum = tmpsum + np.matmul(AR[:, :, j], y[lags + t - j - 1])
#             u[lags + t, :] -= tmpsum
#         u = u[ lags :,:]
#
#         # lagged_y = np.zeros((T - lags, n, lags))
#         # for j in range(lags):
#         #     lagged_y[:, :, j] = y[lagged_y.shape[0] - j - 1:T - j - 1, :]
#         # lagged_AR = np.transpose(AR, (2, 0, 1))
#         # u = y[lags:, :] - np.sum(np.matmul(lagged_AR, lagged_y), axis=2)
#
#
#         return u
#
#     u = get_u(y, AR) # todo: add_const=add_const, add_trend=add_trend, add_trend2=add_trend2, exog=exog,
#
#     T, n = np.shape(u)
#
#     f_u = np.empty([T, lags * n*n])
#     for p in range(lags):
#         y_lag = y[lags - p-1: -p-1, :]
#         # Perform element-wise multiplication
#         result = u[:, :, np.newaxis] * y_lag[:, np.newaxis, :]
#         # Calculate the column indices to fill in f_u
#         start_col = p * (n * n)
#         end_col = p * (n * n) + (n * n)
#         # Fill the appropriate columns of f_u with the result
#         f_u[:, start_col:end_col]= result.reshape((u.shape[0], -1))
#
#
#
#
...
def penalty(lambd, weights,b,V,NormalizeA,SoftRestrictions, restrictions,type='B', blocks=False ,whiten=False ):
    B = SVAR.get_BMatrix(b, restrictions=restrictions, blocks=blocks,     whiten=whiten )
    if whiten:
        B = np.matmul(V, B)
    b = SVAR.get_BVector(B,   whiten=False )-SVAR.get_BVector(SoftRestrictions,   whiten=False )
    if type=='A':
        A = np.linalg.inv(B)
        A = np.matmul(np.linalg.inv(np.diag(A[NormalizeA == 1])), A)
        b = SVAR.get_BVector(A,   whiten=False )-SVAR.get_BVector(SoftRestrictions,   whiten=False )
    b[np.isnan(b)]=0
    penalty = lambd * np.sum(np.multiply(weights, np.power(b,2)))

    return penalty



def get_weights(  b,   SoftRestrictions,V, NormalizeA=[],gamma=1,delta=1e-7,q=0.0,type='B',weightupdate=[],whiten=False):


    B = SVAR.get_BMatrix(b,   whiten=whiten )
    if whiten:
        B = np.matmul(V, B)
    restrictions=SVAR.get_BVector(SoftRestrictions,   whiten=False )
    restrictions[np.isnan(restrictions)]=0
    b = SVAR.get_BVector(B,   whiten=False )-restrictions

    n = np.size(B,1)
    if np.array(weightupdate).size == 0:
        weightupdate =  np.full([n ,1], 1)
    weightupdate = weightupdate.transpose()
    weightupdateB = np.repeat(weightupdate, n, axis=0)
    weightupdateb = SVAR.get_BVector(weightupdateB,   whiten=False )

    if type=='A':
        A = np.linalg.inv(B)
        A = np.matmul(np.linalg.inv(np.diag(A[NormalizeA == 1])), A)
        a = SVAR.get_BVector(A,   whiten=False )-restrictions
        b = a

        weightupdate = weightupdate.transpose()
        weightupdateB = np.repeat(weightupdate, n, axis=1)
        weightupdateb = SVAR.get_BVector(weightupdateB, whiten=False)

    weights = np.zeros(np.shape(b))
    weights[np.abs(b) <= delta] = \
        np.multiply(
            np.power(delta, q - 2),
             np.exp((q - 2) / gamma * np.log(1 + np.power(np.abs(np.divide(b[np.abs(b) <= delta], delta)), gamma)))
                   )
    weights[np.abs(b) > delta] = \
        np.multiply(
            np.power(np.abs(b[np.abs(b) > delta]), q - 2),
             np.exp((q - 2) / gamma * np.log( 1+ np.power(np.abs(np.divide(delta, b[np.abs(b) > delta])), gamma)))
        )

    weights = np.multiply(weights, 1+weightupdateb)
    weights[np.isnan( SoftRestrictions.flatten()) ] = np.zeros(np.sum( np.isnan(SoftRestrictions.flatten()) ))

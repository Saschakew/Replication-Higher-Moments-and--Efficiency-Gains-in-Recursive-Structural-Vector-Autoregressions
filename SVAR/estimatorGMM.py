...
def prepareOptions(u,
                   ...
                   SoftRestrictions=[], signRestA=[],NormalizeA=[],
                   lambd=0, weights=[], Weightopt='I',PenaltyType='B',
    ...
    options['method'] = method  # 'L-BFGS-B' 'trust-constr' 'SLSQP','COBYLA','TNC','Powell'
    ...
    options['Bcetenter'] = Bcetenter
    options['signRestA'] = signRestA
    if np.array(NormalizeA).size == 0:
        NormalizeA = np.eye(n)
    options['NormalizeA'] = NormalizeA
    options['PenaltyType'] = PenaltyType
    options['SoftRestrictions'] = SoftRestrictions

    if np.array(weights).size == 0:
         weights = np.zeros(np.shape(options['bstart']))
...
def SVARout(est_SVAR, options, u):
    ...
    if options['lambd'] != 0:
        loss = SVAR.estimatorGMM.loss(u, SVAR.get_BVector(out_SVAR['B_est'],  restrictions=options['restrictions']),
                                                  ...
                                                  W=options['W'] ) +  SVAR.SVARutilGMM.penalty(options['lambd'], options['weights'],  SVAR.get_BVector(out_SVAR['B_est'],  restrictions=options['restrictions']),

            ...
            for alpha in alphas:
                if np.array(irf_bootstrap).size != 0:
                    if sym:
                        ...
                        med = np.quantile(y_bootstrap, 0.5, axis=0)
...
def plot_IRF2(irf1, irf_bootstrap1=[], irf2=[] ,irf_bootstrap2=[], alphas=np.array([0.2 / 2, 0.32 / 2]), shocks=[], responses=[], shocknames=[], responsnames=[], cummulative=[], sym=False,maxhor=[]):
    n = np.shape(irf1)[1]
    if np.array(shocks).size == 0:
        shocks = np.arange(n)
    if np.array(responses).size == 0:
    ...
    if np.array(maxhor).size == 0:
        maxhor =   np.shape(irf1)[0]
    ...
    for response_idx, response  in enumerate(responses):
        ...
        for shock_idx, shock in enumerate(shocks):
            y1 = irf1[:, response, shock]
            y2 = irf2[:, response, shock]
            try:
                y_bootstrap1 = np.asarray(irf_bootstrap1)[:,:,response,shock]
                y_bootstrap2 = np.asarray(irf_bootstrap2)[:,:,response,shock]
                if cummulative[response]:
                    y_new1 = copy.deepcopy(y1)
                    y_bootstrap_new1 = copy.deepcopy(y_bootstrap1)
                    y_new2 = copy.deepcopy(y2)
                    y_bootstrap_new2 = copy.deepcopy(y_bootstrap2)
                    for t in range(np.size(y)):
                        y_new1[t] = np.sum(y1[0:t + 1])
                        y_bootstrap_new1[:,t]= np.sum(y_bootstrap1[:,0:t+1],axis=1)
                        y_new2[t] = np.sum(y2[0:t + 1])
                        y_bootstrap_new2[:,t]= np.sum(y_bootstrap2[:,0:t+1],axis=1)
                    y1 = copy.deepcopy(y_new1)
                    y_bootstrap1 = y_bootstrap_new1
                    y2 = copy.deepcopy(y_new2)
                    y_bootstrap2 = y_bootstrap_new2
            except:
                if cummulative[response]:
                    y_new1 = copy.deepcopy(y1)
                    y_new2 = copy.deepcopy(y2)
                    for t in range(np.size(y1)):
                        y_new1[t] = np.sum(y1[0:t + 1])
                        y_new2[t] = np.sum(y2[0:t + 1])
                    y1 = copy.deepcopy(y_new1)
                    y2 = copy.deepcopy(y_new2)

            x = np.arange(np.shape(y1)[0])


            ax[ shock_idx,response_idx].plot(x[:maxhor], y1[:maxhor], '--',color="blue")
            ax[ shock_idx,response_idx].plot(x[:maxhor], y2[:maxhor], '--',color="red")
            ax[shock_idx,response_idx].axhline(0, color='black', linewidth=0.5, )
            # Plot Bootstrap irf quantiles
            for alpha in alphas:
                if np.array(irf_bootstrap1).size != 0:
                    if sym:
                        lower1 = np.quantile(y_bootstrap1, alpha, axis=0)
                        upper1 = np.quantile(y_bootstrap1, 1 - alpha, axis=0)
                        med1 = np.quantile(y_bootstrap1, 0.5, axis=0)

                        lower1 =  y1 - (med1 - lower1)
                        upper1 =  y1 + (upper1 - med1)

                    else:
                        lower1= np.quantile(y_bootstrap1, alpha, axis=0)
                        upper1 = np.quantile(y_bootstrap1, 1 - alpha, axis=0)

                if np.array(irf_bootstrap2).size != 0:
                    if sym:
                        lower2 = np.quantile(y_bootstrap2, alpha, axis=0)
                        upper2 = np.quantile(y_bootstrap2, 1 - alpha, axis=0)
                        med2 = np.quantile(y_bootstrap2, 0.5, axis=0)

                        lower2 = y2 - (med2 - lower2)
                        upper2 = y2 + (upper2 - med2)

                    else:
                        lower2 = np.quantile(y_bootstrap2, alpha, axis=0)
                        upper2 = np.quantile(y_bootstrap2, 1 - alpha, axis=0)

                    ax[shock_idx,response_idx].fill_between(x[:maxhor], lower1[:maxhor], upper1[:maxhor], color='blue', alpha=0.25)

                    ax[shock_idx,response_idx].fill_between(x[:maxhor], lower2[:maxhor], upper2[:maxhor], color='red', alpha=0.25)
